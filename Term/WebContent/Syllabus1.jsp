...
<%@page import="java.util.ArrayList"%>     <%@ page import= "java.io.*" %>
...
<nav>
	<ul class="pager XZ">
    <li class="previous"><a href="Sview.jsp"><span >&larr;</span> Back</a></li>
...
<div class="col-lg-3 ">
  ...
  <%} %>
 <li role="presentation" class="fadeInDown animated"><a href="Sview.jsp">Syllabus</a></li>
...
<div class="col-lg-8"><div class="well">
<div class="table-responsive"><br/><br/>
<div class="alert alert-info fadeInUp animated"" role="alert">Syllabus Details</div>
 <% if(user1==null){%>

<tr>    <thead> 
                            <th colspan="3"><div class="well">Upload File</div></th>        
                    </thead>
    
        <form method="post" action="UploadFileController" enctype="multipart/form-data">
            <center>
               <table class="table table-responsive fadeInUp animated"">
                 
                    <tbody></div>
                        <tr>    
                             <td class="danger lead">Title Of File : </td>
                             <td class="danger"><input type="text" name="title" size="30"></td>
                        </tr>    
                        <%
Class.forName("com.mysql.jdbc.Driver");
Connection conxx=DriverManager.getConnection("jdbc:mysql://localhost:3306/mystore","root","root");
String strxx="SELECT * from department";
PreparedStatement psxx=conxx.prepareStatement(strxx);
ResultSet rsxx=psxx.executeQuery();
%>
                         <div class="form-group">
                         <label for="nm22" class="danger lead">Department:</label>
                          <select name="cr" >
			                  <%  while(rsxx.next()){ %>
			                  
			            <option value="<%=rsxx.getString(2)%>"><%=rsxx.getString(2)%></option>       <% } %>
			        </select>  
...
String strx="SELECT * from course";
PreparedStatement psx=conx.prepareStatement(strx);
ResultSet rsx=psx.executeQuery();
%>   
			        <label for="nm22" class="danger lead">Course:</label>
                          <select name="cr1">
			               <%  while(rsx.next()){ %>
			            <option value="<%=rsx.getString(3)%>"><%=rsx.getString(3)%></option>  <% } %>
			        </select>  
...
%>
			          <%
Connection conz=DriverManager.getConnection("jdbc:mysql://localhost:3306/mystore","root","root");
String strz="SELECT * from sem";
PreparedStatement psz=conz.prepareStatement(strz);
ResultSet rsz=psz.executeQuery();
%>  
			        <label for="nm22" class="danger lead">Semester:</label>
                          <select name="cr2">
			                <%  while(rsz.next()){ %>
			            <option value="<%=rsz.getString(3)%>"><%=rsz.getString(3)%></option>  <% } %>
			        </select>  
                        ...
                        <tr>
                             <td class="danger lead">Choose File : </td>
                             <td class="danger"><input type="file" name="file_uploaded" /></td>
                        </tr>
                        <tr>
                             <td class="danger" colspan="3"><center><button type="submit" class="btn btn-info" value="Upload">Upload</center></td>
                        </tr>
                    </tbody>             
                </table>
            </center>
     ...
     <%} %>
       <table class="table table-responsive">
            <thead> 
               <th colspan="3"><div class="well">Uploaded Files</div></th>        
            </thead>
                <tbody>
                    <tr>
                         <td class="danger"><center><b>File Id</b></center>
                          <td class="danger"><center><b>File Title</b></center>
                           <td class="danger"><center><b>File</b></center></td>
                           <td class="danger"><center><b>Action</b></center></td>
                           <% if(user1==null){%>
                           <td class="danger"><center><b>Delete</b></center></td>
                           <%} %>
                    </tr>
                    ...
                    <%
                        try
                        {
                        	...
                        	Connection conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/mystore","root","root");
                                ...
                                ResultSet rs1=myStatement.executeQuery(sqlString);
                            ...
                            %>
                                    <tr>
                                    <td class="success"><center><%out.print(rs1.getString("id")); %></center></td>
                                    <td class="success"><center><%out.print(rs1.getString("title")); %></center></td>
                                    <td class="success"><center><a href="view_file.jsp?id=<%out.print(rs1.getString("id"));%>">View</a></center></td>
                                    <td class="success"><center><a href="file_download.jsp">Download</a></center></td>
                                    <% if(user1==null){%>
                                           <form name=myname method=post action="Sdel.jsp">
 					<td class="info"><input type="checkbox" name="check1<%=iii%>"
							value=<%=rs1.getString(1)%>>
						<input type="submit" value="Delete" class="btn btn-danger fadeInleft animated"></td></form>  <%} %>
                                    </tr>
                            <%
                                }
                            %>
