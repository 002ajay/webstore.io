...
Class.forName("com.mysql.jdbc.Driver");
Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/mystore","root","root");
String str="SELECT stud2.sid,stud2.fname, stud2.ffname, stud2.gender,stud2.email FROM stud2";
...
<div class="alert alert-info fadeInDown animated" role="alert">Manage Students</div>
<a href="SignUp.jsp"><button type="button"class="btn btn-info pull-right bounce animated" > Add</button></a> <br/> <br/>
<form name=myname method=post
action="deleteSuccess1.jsp">
<table class="table table-responsive">
<tr class="fadeInDown animated"> 
	
	 <td class="danger"> <h4> View Profile</h4> </td>
	  <td class="danger"> <h4> Update</h4> </td>
	 <td class="danger"> <h4> Student ID</h4> </td>
     <td class="danger"> <h4> Student Name</h4> </td>
...
while(rs.next()){ %>
			 ...
			 <form name=myname method=post action="ProfileStud.jsp" ><td class="info fadeInDown animated"><input type="image" src="imageDisplay1.jsp?imgid=<%=rs.getInt(1)%>" class="fadeInDown animated img-circle XCXC" width="35" height="35">
			<input type="hidden" name="check<%=ii%>"
			value=<%=rs.getString(1)%>>
			<a herf="#"><input type="submit" value="ViewProfile" class="btn btn-custom CX"></a></td> </form>
			<form name=myname method=post action="UpdateStud.jsp" ><td class="info fadeInDown animated">
			<input type="hidden" name="check<%=ii%>"
			value=<%=rs.getString(1)%>>
			<a herf="#"><input type="submit" value="Edit Profile" class="btn btn-custom CX"></a></td> </form>
		   	 <td class="info fadeInLeft animated">  <%=rs.getString("sid") %> </td>
			 <td class="info fadeInLeft animated">  <%=rs.getString("fname") %> </td>
		 	 <td class="info bounce animated">  <%=rs.getString("ffname") %> </td>
			 <td class="info fadeInRight animated">   <%=rs.getString("gender") %> </td>
			 <td class="info fadeInRight animated">  <%=rs.getString("email") %> </td>
			  <td class="info fadeInRight animated"><input type="checkbox" name="check1<%=i%>"
			value=<%=rs.getString(1)%>>
